...
author: "Susanne McDermott, Alexandra Dowlin, Bethany Riggle, and Margaret Siple"
...
:::notes
This is the fourth year we have instituted some important changes to our survey in an attempt to slow the work pace and reduce the repetitive motion injuries that have been on the rise in recent years.
:::
...
Cpt Arthur McArdle, first year on Aleutian Islands survey
:::
...
:::notes
Both of our charter vessels have conducted the `r SRVY` survey several times and our skippers are experienced fishermen.
...
-   Station allocation based on stratum area and the abundance and variance of 15 representative species
...
(If AI year) We randomly allocate stations within this design framework from a pool of previously successfully trawled locations
...
-   `r SRVY` data are finalized as of **August 27th**
-   Age composition will be updated as Age & Growth finish analyzing otolith samples
-   CPUE tables will be available on FOSS and AKIN
-   GAP_PRODUCTS has been updated with 2026 data products

:::notes
This year we’ve finalized our survey data prior to the September Plan Team meetings and design based estimates along with length composition tables are now available in GAP_PRODUCTS.
...
## Updates since last year's Plan Team presentation

- Emphasis on differences from long-term mean (provides a better idea of stock status relative to expectations)
- Simplified temperature plots
...
```{r catchcomp}
knitr::include_graphics(path = here::here("output/AI_2026/figures/2026_biomass_catchcomp.png"))
```
...
for (i in 1:nrow(report_species)) { # nrow(report_species)
  ...
  is_complex <- grepl(species_code, pattern = "[A-Z]")
  complex_sentence <- ifelse(is_complex,
                             paste("This complex includes:", complex_name_text$species[which(complex_name_text$complex==species_code)]),
                             "")
  cpue_map_path <- here::here(paste0("output/", SRVY, "_", maxyr, "/","figures/",maxyr,"_",spp_name_informal,"_bubble.png"))
...
## Biomass compared to the long-term mean

```{r}
knitr::include_graphics(path = here::here("output/AI_2026/tables/PercentChangeTable2.png"))
```
