...
```{r fig.cap='Bottom temperatures as measured at the trawl. Brown points represent the median.', fig.id='bottom-temp',  fig.cap.style = "Figure Caption", fig.width=6}
...
for (i in 1:3) { # nrow(report_species)
  ...
  octopus <- species_code!=78403 # true/false, if it's for octopus don't make length comp or length scatter figs
  ...
  depth_longest <- meanlengths_depth %>%
    dplyr::filter(SPECIES_CODE == species_code) %>%
    top_n(1, `Mean length`) %>%
