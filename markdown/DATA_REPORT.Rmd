...
sp_prices %>%
  ...
  dplyr::select(-`Included in design`, -Source) %>%
  flextable() %>%
  fix_border_issues() %>%
  theme_vanilla() %>%
...
```{r fig.cap='Surface temperatures as measured at the trawl. Larger brown points represent the median, small black points represent individual hauls. Shaded distributions represent the density of measurements. Dotted lines show the mean surface temperature over the past 10 and 20 years.', fig.id='surface-temp',  fig.cap.style = "Figure Caption", fig.width=6}
