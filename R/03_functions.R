...
make_tab3 <- function(species_code = NULL, survey = NULL, year = NULL){
  if(survey=="AI"){
    ...
    a <- RODBC::sqlQuery(channel, paste0("define xsurvey_area = 'AI' define b = 'ai.biomass_' define sz = 'ai.sizecomp_' define in = 'inpfc' define in2 = 'inpfc_' define a = 'area' define a2 = 'area_' define d = 'depth' define t = 'total' define cpue = 'ai.cpue' define bindy = &b&in2&d
define bdy = &b&d
define biny = &b&in
define bay = &b&a
define bady = &b&a2&d
...
select b.species_code, b.summary_area_depth summary_area, haul_count, catch_count, round(b.mean_wgt_cpue, 0) mean_wgt_cpue,
	...
	where b.year = &", year, " and s.year = &", year,
...
insert into biomass_tab select b.species_code, b.summary_area, haul_count, catch_count, 
  ...
  round((area_biomass*1000)/area_pop,3) weight
	from &biny b, &sziny s
	where b.year = &",year," and s.year = &",year, "and b.summary_area = s.summary_area(+)
	and b.species_code = s.species_code(+) and area_pop > 0
