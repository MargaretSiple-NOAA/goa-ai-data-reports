...
readtext2 <- function(file, refcontent = FALSE) {
  ...
  if (insert == "") { # if there is nothing in the doc, no problem!
    ...
    if (refcontent) {
      insert <- paste0(
        "**Hand written text from google drive**: ",
    ...
    insert[substr(start = 1, stop = 2, x = insert) == "r "] <-
      paste0(substr(
        x = insert[substr(start = 1, stop = 2, x = insert) == "r "],
        start = 3,
...
species_text <- function(table_spp_print,
                         haul0,
                         biomass_cpue,
                         length_maxyr0,
                         # spp_sci,
...
During the ", maxyr, " survey, ", spp_print, " were present at ",
    formatC(
      x = (length(unique(haul_maxyr_spp$hauljoin)) / length(unique(haul_maxyr0$hauljoin))) * 100,
      digits = 1, format = "f"
    ),
    "% of stations in the ",
...
The ",
  ...
  temp <- function(biomass_cpue, biomass_cpue_spp, maxyr, compareyr, tempyr,
    ...
    str0 <- paste0(
      ...
      NMFSReports::pchange(
        start = sum(biomass_cpue_spp[biomass_cpue_spp$year == compareyr, metric], na.rm = TRUE),
      ...
      NMFSReports::pchange(
        start = sum(biomass_cpue_spp[biomass_cpue_spp$year == tempyr, metric], na.rm = TRUE),
...
#' @examples
#' tolower2(
#'   str0 = "notice how there are built-in proper nouns are capitalized:
...
add_report_spp <- function(spp_info,
  ...
  # all -> other
  temp <- unique(temp1$print_name)[grepl(
    pattern = "all ",
  ...
  if (length(temp) > 0) {
    for (i in 1:length(temp)) {
      temp2 <- intersect(
        temp1$species_code[temp1$print_name == temp[i]],
        temp1$species_code[temp1$print_name != temp[i]]
...
plot_temps_facet <- function(rasterbrick,
  ...
  cbar_legend <- legend_discrete_cbar(
...
#' @export
legend_discrete_cbar <- function(breaks, # Vector of breaks. If +-Inf are used, triangles will be added to the sides of the color bar
                                 palette = "Greys", #
  ...
  if (any(inf_breaks == 1)) { # Add < arrow for -Inf
    ...
    cbar_plot <- cbar_plot +
      geom_polygon(
        data = polystart, aes(x = x, y = y * expand.y),
        show.legend = FALSE,
        inherit.aes = FALSE,
        fill = colors[1],
        color = border_color
  ...
  if (any(inf_breaks > 1)) { # Add > arrow for +Inf
    ...
    cbar_plot <- cbar_plot +
      geom_polygon(
        data = polyend, aes(x = x, y = y * expand.y),
        show.legend = FALSE,
        inherit.aes = FALSE,
  ...
  if (!is.null(legend_title)) { # Add legend title
    cbar_plot <- cbar_plot +
      annotate(
        geom = "text", x = legend_position, y = title_pos,
        label = legend_title,
        angle = angle,
        size = font_size,
        family = font.family
...
plot_sizecomp <- function(sizecomp0,
  ...
  if (!ridgeline) { # facet plot without ridgeline
    ...
    figure <- ggplot(
      ...
      scale_x_continuous(
        name = stringr::str_to_sentence(paste0(type, " (", len_unit_word, ")")),
    ...
    temp <- setdiff(
      as.numeric(paste(min(table_raw1$year, na.rm = TRUE))):as.numeric(paste(max(table_raw1$year, na.rm = TRUE))),
  ...
  if (print_n & !is.null(length_data0)) {
    dat_text <- data.frame(
      label = paste0(
        c("# measured: ", rep_len(x = "", length.out = (length(unique(length_data0$year)) - 1))),
        length_data0 %>%
          dplyr::mutate(year = as.character(year)) %>%
          dplyr::ungroup() %>%
          dplyr::group_by((year)) %>%
          dplyr::summarise(frequency = formatC(
            x = sum(frequency, na.rm = TRUE),
...
plot_timeseries <- function(dat,
  ...
  figure <- figure +
    ...
    xlab("Year") +
    scale_y_continuous(
      name = paste0(stringr::str_to_sentence(spp_print), "\n", y_long, unit_word),
...
tag_facet <- function(p, open = "(", close = ")", tag_pool = letters, x = -Inf, y = Inf,
  ...
  p + geom_text(
    data = tags, aes_string(x = "x", y = "y", label = "label"), ..., hjust = hjust,
...
plot_survey_stations <- function(reg_dat,
  ...
  if (study) {
    ...
    figure <- figure +
      geom_sf(
        data = reg_dat$survey.grid %>% dplyr::filter(!is.na(study)),
  ...
  if (station_pts_vess) {
    ...
    figure <- figure +
      ...
      geom_sf(
        ...
        aes(
          color = reg_dat$survey.area$SURVEY,
      ...
      scale_color_manual(
        name = " ", # "Survey Region",
        values = c(
          alpha(colour = c(survey_reg_col), 0.7),
          unique(vess$vess_col)
        ),
        breaks = c(
          rev(unique(reg_dat$survey.area$SURVEY)),
          unique(vess$vess_col)
        ),
        labels = c(
      ...
      scale_shape_manual(
        ...
        breaks = c(
          unique(reg_dat$survey.area$SURVEY),
          unique(vess$vess_shape)
        ),
        labels = c(
          rev(unique(stringr::str_to_title(haul_cruises_maxyr$SRVY_long))),
  ...
  figure <- figure +
    ...
    geom_text(
      data = subset(reg_dat$place.labels, type == "mainland"),
      aes(x = x, y = y, label = lab),
      size = 14, group = 99
    ) +
    geom_shadowtext(
      data = subset(reg_dat$place.labels, type == "peninsula"),
...
table_change <- function(dat,
  ...
  if (length(yrs) == 4) {
    a <- Reduce(
      intersect,
      list(
        which(table_raw[, as.character(yrs)[1]] == 0),
  ...
  table_print <- table_print %>%
    ...
    dplyr::mutate(
      ...
      species_name1 =
        gsub(
          pattern = " sp.", replacement = "",
          x = species_name1, fixed = TRUE
        ),
      species_name1 =
        gsub(
          pattern = " spp.", replacement = "",
          x = species_name1, fixed = TRUE
        ),
  ...
  table_print <- table_print %>%
    flextable::flextable(data = ., col_keys = c(
      ifelse(length(unique(table_print$SRVY)) == 1, "", "Survey"),
      "group", "dummy",
    ...
    ) %>% # https://stackoverflow.com/questions/57474647/italic-and-color-in-an-r-flextable
    flextable::color(
      color = "red",
...
table_change_pres <- function(dat,
  ...
  for (i in 1:nrow(temp)) {
    table_print[, as.character(temp$var1)[i]] <-
      paste0(
        unlist(table_print[, as.character(temp$var1)[i]]),
        " (",
...
check_outlier <- function(species_code, year, catch_data, plot = FALSE){
  ...
  if(plot & length(o) > 0){
    ...
    p <- ggplot() +
      ...
      geom_point(data = sp_catch,
                 aes(x = START_LONGITUDE, y = START_LATITUDE), cex = 1) +
      geom_point(data = o, aes(x = START_LONGITUDE, y = START_LATITUDE), 
                 col = "red", cex = 1.5) +
      ggtitle(label = sp)
    print(p)
  }
